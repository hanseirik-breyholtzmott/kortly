...
export default function Leaderboard() {
  ...
  const allUsers = [
    { id: "1", username: "AshKetchum", email: "ash@pokemon.com", joinedAt: "2024-01-15" },
    { id: "2", username: "MistyWater", email: "misty@pokemon.com", joinedAt: "2024-02-01" },
    { id: "3", username: "BrockRock", email: "brock@pokemon.com", joinedAt: "2024-02-10" },
    { id: "4", username: "GaryOak", email: "gary@pokemon.com", joinedAt: "2024-01-20" },
    { id: "5", username: "TeamRocket", email: "rocket@pokemon.com", joinedAt: "2024-02-15" },
  ...
  const getRankBadge = (rank: number) => {
    switch (rank) {
      ...
      case 3:
        return <Badge className="bg-amber-100 text-amber-800 border-amber-300">3rd Place</Badge>
      default:
        return null
    }
  }
    ...
    <div className="space-y-6">
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-2xl font-bold text-blue-900">#{currentUserRank.rank}</span>
                </div>
                <div>
                  <h3 className="font-semibold text-blue-900">Your Ranking</h3>
                  <p className="text-sm text-blue-700">
                    {currentUserRank.uniquePokemon} unique Pokemon • {currentUserRank.totalCards} total cards
                  </p>
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        {leaderboardData.slice(0, 3).map((trainer, index) => {
          ...
          return (
            <Card
              key={trainer.id}
              className={`${getCardColor(trainer.rank, isCurrentUser)} shadow-lg transform transition-all hover:scale-105`}
            >
              <CardContent className="p-6 text-center">
                ...
                <h3 className="font-bold text-lg text-gray-900 mb-1">{trainer.username}</h3>
                {getRankBadge(trainer.rank)}
                <div className="mt-4 space-y-2">
                  <div className="flex justify-between text-sm">
                    <span className="text-gray-600">Unique Pokemon:</span>
                    <span className="font-bold text-blue-600">{trainer.uniquePokemon}</span>
                  </div>
                  <div className="flex justify-between text-sm">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="font-bold text-purple-600">{trainer.rareCards}</span>
                  </div>
                </div>
              </CardContent>
            </Card>
      ...
      <Card className="bg-white/90 backdrop-blur shadow-lg">
        <CardHeader>
          ...
          <CardDescription>Complete ranking of all Pokemon trainers</CardDescription>
        </CardHeader>
                ...
                >
                  <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 min-w-[60px]">
                      ...
                      <span className="text-xl font-bold text-gray-900">#{trainer.rank}</span>
                    </div>
                  ...
                  <div className="flex items-center gap-6 text-sm">
                    <div className="text-center">
                      <div className="font-bold text-blue-600 text-lg">{trainer.uniquePokemon}</div>
                      <div className="text-gray-600">Unique</div>
                    </div>
                    <div className="text-center">
                      <div className="font-bold text-gray-900 text-lg">{trainer.totalCards}</div>
                      <div className="text-gray-600">Total</div>
                    </div>
                    <div className="text-center">
                      <div className="font-bold text-purple-600 text-lg">{trainer.rareCards}</div>
